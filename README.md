...
[Quickstart](#run-locally) • [Runtime Modes](#runtime-modes-streamlit-first) • [Release Notes](#what's-new-in-v095)

Agentora is a **local-first multi-agent runtime** with memory, collaboration, workers, and now safe desktop/browser automation. Streamlit provides the primary operator UX, and FastAPI provides orchestration, policy, and execution APIs.


## What’s new in v0.9.5
- Adds Desktop Actions and Browser Actions with explicit policy evaluation and approval gates.
- Adds Action Requests, approvals, executions, artifacts, workflows, and workflow run history APIs.
- Adds Action-aware team orchestration so subgoals can trigger bounded desktop/browser steps.
- Adds worker-assisted action routing and lifecycle traces for local-vs-worker execution decisions.
- Adds a compact Streamlit Action Center for pending approvals, action history, workflows, and replay.
